...
int VideoEncodeH264::initVideoEncoder() {
    ...
    avformat_alloc_output_context2(&pFormatCtx, NULL, "h264", out_file);

    if(pFormatCtx == NULL)
    {
        LOGE(1, "pFormatCtx==null");
        return -1;
    }
    ...
    LOGI(1, "initVideoEncoder avformat_new_stream  4444");
...
//        av_dict_set(&param, "profile", "baseline", 0);
    ...
    LOGI(1, "after avcodec_open2, pix_fmt %d, width %d, height %d.",pCodecCtx->pix_fmt,pCodecCtx->width,pCodecCtx->height);
...
//        av_free(picture_buf);
    ...
    avio_close(pFormatCtx->pb);
    avformat_free_context(pFormatCtx);
    LOGI(1, "VideoEncodeH264::encodeEnd");

    MediaMuxer *pMuxer = new MediaMuxer();
    pMuxer = (MediaMuxer*)arguments->handler;
    
    pMuxer->setup_video_state(END_STATE);
    pMuxer->try_encode_over(arguments);
    delete pMuxer;
    return 1;
}
