...
int AudioEncodeAAC::initAudioEncoder() {
    LOGI(1,"initAudioEncoder audio_path %s", arguments->audio_path);
    ...
    if (pFormatCtx == NULL) {
        LOGE(1,"initAudioEncoder pFormatCtx = NULL!");
        return -1;
    }
    fmt = av_guess_format(NULL, out_file, NULL);
    if (fmt == NULL) {
        LOGE(1,"initAudioEncoder fmt = NULL!");
        return -1;
    }
